PKGNAME := solutionstudio
SPECFILE := $(PKGNAME).spec
VERSION := $(shell awk '/Version:/ { print $$2 }' $(SPECFILE))
RELEASE := $(shell awk '/Release:/ { print $$2 }' $(SPECFILE) | sed -e 's|%{?dist}||g')

SUBDIRS = bin docsrc/man etc share solutionstudio
...
bumpver:
	...
	changelog="`hg log --exclude .hgtags --exclude solutionstudio.spec --exclude ChangeLog --exclude Makefile -r tip:$(PKGNAME)-$(VERSION)-$(RELEASE) --template "- {desc|strip|firstline} ({author})\n" 2> /dev/null || echo "- Initial Build"`"; \
