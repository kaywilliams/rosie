...
bumpver:
	...
	changelog="`hg log --exclude .hgtags --exclude systemstudio.spec --exclude ChangeLog --exclude Makefile -r tip:$(PKGNAME)-$(VERSION)-$(RELEASE) --template "- {desc|strip|firstline} ({author})\n" 2> /dev/null || echo "- Initial Build"`"; \
