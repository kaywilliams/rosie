...
opt_defaults = dict(
  ...
  skip_events   = [],
  skip_test = [],       
  mainconfigpath = '/etc/centosstudio/centosstudio.conf',
  enabled_modules  = [],
  disabled_modules = [],
  list_modules = False,
  list_events = False,
...
def parse_cmd_args(defaults=None):
  ...
  parser.add_option('--centosstudio-conf', metavar='PATH',
    dest='mainconfigpath',
  ...
  parser.add_option('--share-path', metavar='PATH',
    dest='sharepath',
    action='append',
    help='specify directory containing centosstudio share files')
...
def main():
  ...
  cstest.BUILD_ROOT = pps.path(options.buildroot)
  cstest.EventTestCase.options = options

  runner = cstest.EventTestRunner(options.testlogfile, options.testloglevel)
  ...
  try:
    ...
    if options.clear_test_cache:
      cstest.BUILD_ROOT.rm(recursive=True, force=True)
