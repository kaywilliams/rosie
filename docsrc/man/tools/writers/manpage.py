...
class Translator(nodes.NodeVisitor):
    ...
    def visit_Text(self, node):
        ...
        text = text.replace("'","\\'")
        if ( not self._in_title and self.section_level >= 3
             and not self._add_re.get(self.section_level, False) ):
            text = '.RS (\\n(SNu)\n.PP\n%s\n' % text
            self._add_re[self.section_level] = True
    ...
    def depart_section(self, node):
        if self._add_re.get(self.section_level, False):
            self.body.append('.RE\n')
            self._add_re[self.section_level] = False
        self.section_level -= 1
