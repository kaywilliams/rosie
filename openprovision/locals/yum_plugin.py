...
PLUGIN_DEFAULTS = {
  ...
  'plugin': [
    ...
    '',
    '    ##### sync gpgkeys #####',
    '    base.verbose_logger.info(_(\'Synchronizing keys\') ) ',
    '',
    ...
    '      base.verbose_logger.info(_(\'Unable to retrieve GPG key list: %s\') % ',
    ...
    '        keyurl = (repo.urls[0] + \'gpgkeys/\' + keyfile)',
    '        try:',
    '          rawkey = ug.urlread(keyurl)',
    '        except URLGrabError, e:',
    '          raise YumBaseError(_(\'GPG key retrieval failed: %s\') % ',
    '                                 (yum.i18n.to_unicode(str(e))))',
    '',
    '        try:',
    ...
    '          keys_changed = True',
    '          base.verbose_logger.info(_(\'Importing key %s "%s" \') %',
    ...
    '          keys_changed = True',
    '          base.verbose_logger.info(_(\'Removing GPG key %s "%s" \') %',
    ...
    '',
    '      if not keys_changed:',
    '        base.verbose_logger.info(_(\'Nothing to do\'))',
    '',
    '    else:',
    '      base.verbose_logger.info(_(\'No keys to sync\'))',
    '',
    '    ##### sync packages #####',
    '    base.verbose_logger.info(_(\'Synchronizing packages\') ) ',
    '',
    '    try:',
    '      # get packages that have changed',
    '      oldpkgs_all = [ \'.\'.join(str(x).split(\'.\')[:-1]) for x in',
    '                      base.doPackageLists(\'extras\').extras]',
    '      newpkgs_all = [ \'.\'.join(str(x).split(\'.\')[:-1]) for x in',
    '                      base.doPackageLists(\'available\', showdups=True).available]',
    '',
    '      # now filter out any packages where only the arch has changed',
    '      oldpkgs = set(oldpkgs_all).difference(set(newpkgs_all))',
    '      newpkgs = set(newpkgs_all).difference(set(oldpkgs_all))',
    '',
    '      # remove any installed package that is not available',
    '      for pattern in oldpkgs:',
    '        base.verbose_logger.info("[%s] removing installed package \'%s\'" % (PLUGIN_NAME, pattern))',
    '        base.remove(pattern=pattern)',
    '',
    '      # add any available package that is not yet installed',
    '      for pattern in newpkgs:',
    '        base.verbose_logger.info("[%s] adding available package \'%s\'" % (PLUGIN_NAME, pattern))',
    '        base.install(pattern=pattern)',
    '',
