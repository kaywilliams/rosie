...
Python script defining the Build class, the primary controller for the Repo Studio.
"""
...
from repostudio.util import dispatch
from repostudio.util import listfmt
from repostudio.util import lock
from repostudio.util import pps
from repostudio.util import rxml
from repostudio.util import shlib
from repostudio.util import si

from repostudio.util import sync
from repostudio.util.sync import cache
from repostudio.util.sync import link
...
DEFAULT_CACHE_DIR = pps.path('/var/cache/repostudio')
DEFAULT_LIB_DIR = pps.path('/var/lib/repostudio')
DEFAULT_SHARE_DIR = pps.path('/usr/share/repostudio')
DEFAULT_LOG_FILE = pps.path('/var/log/repostudio.log')
...
class Build(CentOSStudioEventErrorHandler, CentOSStudioValidationHandler, object):
  ...
  def __init__(self, options, arguments, callback=None):
    ...
    if self.mainconfig.pathexists('/repostudio/debug'):
      self.debug = self.mainconfig.getbool('/repostudio/debug', False)
    if callback: callback.set_debug(self.debug)
    ...
    try:
      self.dispatch = dispatch.Dispatch(
                      loader.load(import_dirs, prefix='repostudio/modules',))
    ...
    # set up locking
    self._lock = lock.Lock( 'repostudio-%s.pid' % self.repoid )
  ...
  def _get_config(self, options, arguments):
    """
    Gets the repostudio config based on option values. The repostudio
    ...
    if cp and cp.exists():
      ...
      self.logger.log(4, "No repostudio config file found at '%s'. Using default settings" % cp)
      mc = rxml.config.fromstring('<repostudio/>')
  ...
  def _get_definition(self, options, arguments):
    ...
    `repostudio -h` on the command line without giving the '-c' option.) 
    """
  ...
  def _compute_import_dirs(self, options):
    ...
    import_dirs = [ x.expand().abspath() for x in \
      self.mainconfig.getpaths('/repostudio/lib-path/text()', []) ]
  ...
  def _compute_modules(self, options):
    """
    Compute a list of modules repostudio should not load.  Disabling takes priorty
    over enabling.
    ...
    for module in self.mainconfig.xpath('/repostudio/enable-module', []):
      enabled.add(module.text)
    for module in self.mainconfig.xpath('/repostudio/disable-module', []):
  ...
  def _compute_event_attrs(self, options):
    ...
    cache_max_size = self.mainconfig.getxpath('/repostudio/cache/max-size/text()', '30GB')
