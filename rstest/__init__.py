...
from repostudio.util import logger
from repostudio.util import pps
from repostudio.util import repo
from repostudio.util import shlib

from repostudio.util.rxml import config

from repostudio.main import Build

BUILD_ROOT = '/tmp/rstest' # location builds are performed
...
class TestBuild(Build):
  ...
  def _get_config(self, options, arguments):
    mcf = pps.path(options.mainconfigpath or '/etc/repostudio/rstest.conf')
    if mcf.exists():
      self.mainconfig = config.parse(mcf).getroot()
    else:
      self.mainconfig = config.parse(StringIO('<repostudio/>')).getroot()
