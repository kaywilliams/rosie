...
from repostudio.util.CleanHelpFormatter import CleanHelpFormatter
...
opt_defaults = dict(
  ...
  skip_events   = [],
  skip_test = [],       
  mainconfigpath = '/etc/repostudio/repostudio.conf',
  enabled_modules  = [],
  disabled_modules = [],
  list_modules = False,
  list_events = False,
...
def parse_cmd_args(defaults=None):
  ...
  parser.add_option('--repostudio-conf', metavar='PATH',
    dest='mainconfigpath',
    help='specify path to a main config file')
  parser.add_option('--lib-path', metavar='PATH',
    dest='libpath',
    action='append',
    help='specify directory containing repostudio library files')
  parser.add_option('--share-path', metavar='PATH',
    dest='sharepath',
    action='append',
    help='specify directory containing repostudio share files')
...
def main():
  ...
  rstest.BUILD_ROOT = pps.path(options.buildroot)
  rstest.EventTestCase.options = options

  runner = rstest.EventTestRunner(options.testlogfile, options.testloglevel)
  ...
  try:
    ...
    if options.clear_test_cache:
      rstest.BUILD_ROOT.rm(recursive=True, force=True)
