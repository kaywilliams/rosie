...
from sstest        import EventTestCase, decorate
...
class DeployMixinTestCase:
  ...
  def __init__(self, distro, version, arch, module=None):
    ...
    deploy = rxml.config.parse(
      '%s/../../share/systemstudio/templates/virt-deploy.xml' %  
    ...
    # update module
    self.hostname = "sstest-%s-%s-%s" % (self.moduleid, self.version, self.arch)
    self.domain = '.local'
