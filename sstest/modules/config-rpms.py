...
from sstest.mixins   import (MkrpmRpmBuildMixinTestCase, RpmCvarsTestCase,
                             DeployMixinTestCase,
...
class ConfigRpmEventTestCase(MkrpmRpmBuildMixinTestCase, EventTestCase):
  ...
  def _make_repos_config(self):
    ...
    base = repo.getDefaultRepoById('base', distro=self.distro,
                                           ...
                                           baseurl='http://www.systemstudio.org/mirrors/%s' % self.distro)
...
class Test_ErrorOnDuplicateIds(ConfigRpmEventTestCase):
  ...
  def runTest(self):
    unittest.TestCase.failUnlessRaises(self, SystemStudioError, 
...
class Test_FilesPersistOnLibDirChanges(DeployConfigRpmEventTestCase):
  ...
  def runTest(self):
    self.event.test_lib_dir = pps.path('/root/systemstudio')
