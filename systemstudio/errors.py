...
class SystemStudioError(Exception): pass

class InvalidOptionError(SystemStudioError):
...
class InvalidConfigError(SystemStudioError):
...
class SystemStudioEventError(SystemStudioError):
  message = None
...
class SimpleSystemStudioEventError(SystemStudioEventError):
  message = "%(message)s\n"

class SystemStudioIOError(SystemStudioEventError, IOError):
  message = "Cannot read file '%(file)s': [errno %(errno)d] %(message)s"

class PpsPathError(SystemStudioEventError):
  def __str__(self):
    if self.error.errno == 21: # EISDIR
      pass

class ShLibError(SystemStudioEventError):
  def __init__(self, e):
...
class RhnSupportError(RuntimeError, SystemStudioEventError):
  def __str__(self):
