...
Python script defining the Build class, the primary controller for the System Studio.
"""
...
from systemstudio.errors    import (SystemStudioEventErrorHandler, 
                                    SystemStudioEventError,
                                    SystemStudioError,
                                    InvalidOptionError,
                                    InvalidConfigError)
from systemstudio.event     import Event, CLASS_META
from systemstudio.cslogging import make_log, L0, L1, L2
...
DEFAULT_LIB_DIR = pps.path('/var/lib/systemstudio')
...
class Build(SystemStudioEventErrorHandler, SystemStudioValidationHandler, object):
  """
  ...
  def __init__(self, options, arguments, callback=None):
    ...
    if self.mainconfig.pathexists('/systemstudio/debug'):
    ...
    try:
      ...
      raise SystemStudioError("Validation of %s failed. %s" % 
    ...
    try:
      ...
      raise SystemStudioError("Error opening log file for writing: %s" % e)
    if callback: callback.set_logger(self.logger)
    ...
    try:
      ...
      raise SystemStudioError("Error loading core systemstudio files: %s" % 
            traceback.format_exc())

    except InvalidEventError, e:
      raise SystemStudioError("\n%s" % e)
    ...
    # set up locking
    self._lock = lock.Lock( 'systemstudio-%s.pid' % self.repoid )
  ...
  def main(self):
    ...
    if self._lock.acquire():
      ...
      raise SystemStudioError("\nAnother instance of systemstudio (pid %d) is "
                              "already modifying '%s'" % 
  ...
  def _compute_events(self, modules=None, events=None):
    ...
    for moduleid in modules or []:
      try:
        ...
        raise SystemStudioError("Module '%s' does not exist or was not loaded"
                                % moduleid)
    r.update(events or [])
    return r
  ...
  def _set_status(self, eventid, status, str):
    ...
    try:
      ...
      raise SystemStudioError("Unregistered event '%s'" % eventid)
    if not e._check_status(status):
      raise SystemStudioError("Cannot %s protected event '%s'" % (str, eventid))
    e.status = status
  ...
  def _compute_event_attrs(self, options):
    ...
    cache_max_size = self.mainconfig.getxpath('/systemstudio/cache/max-size/text()', '30GB')
