...
from systemstudio.errors    import assert_file_has_content, SystemStudioEventError
from systemstudio.event     import Event
from systemstudio.cslogging import L1

from systemstudio.modules.shared import DepsolverMixin, ShelveMixin

from systemstudio.util.depsolver.depsolver import DepsolveError
...
class DepsolveEvent(DepsolverMixin, ShelveMixin):
  ...
  def run(self):
    ...
    try:
      ...
      raise SystemStudioDepsolveError(str(e))
...
class InvalidPkglistFormatError(SystemStudioEventError):
  message = ( "Invalid format '%(pkgfile)s' on line %(lino)d of "
              "pkglist '%(line)s'.\n\nFormat should "
              "be %{NAME}-%{VERSION}-%{RELEASE}-%{ARCH}" )

class SystemStudioDepsolveError(SystemStudioEventError):
