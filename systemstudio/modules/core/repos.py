...
from systemstudio.errors   import assert_file_has_content, assert_file_readable, SystemStudioEventError
...
class ReposEvent(RepoEventMixin, Event):
  ...
  def setup(self):
    ...
    if self.config.pathexists('.'):
      updates.add_repos(ReposFromXml(self.config.getxpath('.'), cls=SystemStudioRepoGroup))
...
#------ ERRORS ------#
class InstallerRepoNotFoundError(SystemStudioEventError):
...
class TreeinfoNotFoundError(SystemStudioEventError):
  message = ( "Unable to find '.treeinfo' file in '%(repoid)s' repo "
              "at '%(repourl)s' " )

class UnsupportedInstallerRepoError(SystemStudioEventError):
