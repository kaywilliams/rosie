...
from systemstudio.cslogging    import MSG_MAXWIDTH, L0, L1, L2
...
from systemstudio.modules.shared import (DeployEventMixin, ShelveMixin, 
                                         RpmBuildMixin) 
...
class SrpmBuildMixinEvent(RpmBuildMixin, DeployEventMixin, ShelveMixin, Event):
  ...
  def run(self):
    ...
    # initialize builder
    # doing this in a method so sstest can call it directly
  ...
  def _get_srpm_from_script(self, script):
    ...
    if not results:
      message = ("The script provided for the '%s' srpm did not output an "
                 ...
                 "System Studio documentation for information on using the "
...
# -------- Error Classes --------#
class SrpmBuildEventError(SystemStudioEventError): 
  message = ("%(message)s")
...
class DefinitionNotFoundError(SrpmBuildEventError):
  message = ("No SRPM build machine definition template found. Please see the "
             "System Studio documentation for information on specifying SRPM "
