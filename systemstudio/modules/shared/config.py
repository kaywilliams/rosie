...
from systemstudio.errors   import (SystemStudioEventError, MissingIdError,
                                   DuplicateIdsError)
...
for f in $files; do
  if [ ! -e $s/$f ]; then # file missing from source folder
    if [ -e $f ]; then    #file exists on disk
      ...
      # find md5files for other systemstudio managed rpms
...
# -------- Error Classes --------#
class ConfigRpmEventError(SystemStudioEventError): 
  message = "%(message)s"
