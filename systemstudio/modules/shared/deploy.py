...
from systemstudio.modules.shared import (InputEventMixin, ExecuteEventMixin,
                                         ScriptFailedError,
                                         SSHFailedError, SSHScriptFailedError)
# InputEventMixin loads ExecuteEventMixin

from systemstudio.util.graph import DirectedNodeMixin
  ...
  def run(self):
    ...
    if self._reinstall():
      if hasattr(self, 'test_fail_on_reinstall'): #set by test cases
        raise SystemStudioError('test fail on reinstall')
...
class InvalidTriggerNameError(SystemStudioEventError):
