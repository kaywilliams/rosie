...
from systemstudio.callback   import LinkCallback
from systemstudio.errors     import SystemStudioEventError
from systemstudio.errors     import SimpleSystemStudioEventError
  ...
  def setup(self):
    ...
    for key in ['%{url}', '%{hostname}', '%{domain}', '%{fqdn}', '%{password}', 
      ...
      if key in map[key]['value']:
        raise SimpleSystemStudioEventError(
          "Macro Resolution Error: \'%s\' macro not allowed in \'%s/%s\'." %
          (key, self.moduleid, map[key]['conf']))
    self.macros = {} # making this an instance attr so sstest can access
    for key in map:
...
#------ Error Classes ------#
class InterfaceIOError(SystemStudioEventError):
  message = ( "Error looking up information for interface '%(interface)s': "
              "%(message)s" )

class InvalidHostnameError(SystemStudioEventError):
  message = "Invalid Hostname: %(message)s"

class FQDNNotFoundError(SystemStudioEventError):
  message = ( "Unable to locate a fully-qualified domain name (FQDN) for "
              ...
              "System Studio documentation on 'Publish' for more information.") 

class KeyGenerationFailed(SystemStudioEventError):
  message = "%(message)s"
