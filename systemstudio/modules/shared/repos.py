...
from systemstudio.errors    import (SystemStudioEventError,
                                    SystemStudioIOError, RhnSupportError,
                                    assert_file_readable, 
                                    assert_file_has_content)
from systemstudio.cslogging    import L1, L2
from systemstudio.constants    import BOOLEANS_TRUE, BOOLEANS_FALSE
from systemstudio.event.fileio import InputFileError
...
__all__ = ['RepoSetupEventMixin', 'RepoEventMixin', 'SystemStudioRepo', 
           'SystemStudioRepoGroup', 'SystemStudioRepoFileParseError']
...
class RepoEventMixin(Event):
  ...
  def validate(self):
    # if type == system, repos config must contain at least one repo or repofile
  ...
  def setup_repos(self, repos=None):
    ...
    if type == "system" and not len(self.repos) > 0:
...
class NoReposEnabledError(SystemStudioEventError, RuntimeError):
  message = "No enabled repos in '%(modid)s' module"

class RepodataNotFoundError(SystemStudioEventError, RuntimeError):
  message = "Unable to find repodata folder for repo '%(repoid)s' at '%(url)s'"

class InvalidRepomdFileError(SystemStudioEventError, RuntimeError):
  message = "%(message)s"

class InconsistentRepodataError(SystemStudioEventError, RuntimeError):
...
class SystemidUndefinedError(SystemStudioEventError, InvalidConfigError):
  message = "No <systemid> element defined for repo '%(repoid)s'"

class SystemidInvalidError(SystemStudioEventError):
...
class SystemStudioRepoFileParseError(SystemStudioEventError):
  message = "Error parsing repo file: %(message)s"

class RepomdCsumMismatchError(SystemStudioEventError):
  message = ( "Checksum of file '%(file)s' doesn't match repomd.xml for "
              "repo '%(repoid)s':\n"
              "  Got:      %(got)s\n"
              "  Expected: %(expected)s" )

class MirrorlistFormatInvalidError(SystemStudioEventError):
